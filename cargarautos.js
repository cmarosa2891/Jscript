...
const camion = {
    ...
    cargarauto(pesoauto, patente) {
        ...
        if ((pesoauto + this.pesocarga < this.pesom)) {   //patente !== this.patentes.find(patente) && 
        ...
        } else { alert(`No se procederá con la carga. Carga actual ${this.pesocarga} peso vehículo: ${pesoauto} peso máximo: ${this.pesom} - Excedente: ${this.pesom - this.pesocarga + pesoauto}`)}
    },
    ...
    consultapantentes(patverificar){
        if(patverificar === this.patentes.find(patverificar)){
